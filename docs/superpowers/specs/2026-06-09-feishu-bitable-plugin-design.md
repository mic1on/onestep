...
插件使用独立 workflow：`.github/workflows/plugin-feishu-bitable.yml`。

PR 和 push 会在插件目录、core runtime、root `pyproject.toml`、`uv.lock` 或 workflow
变化时运行插件测试和插件包构建：

```bash
uv sync --frozen --all-packages --extra test
uv run --all-packages python -m pytest -q plugins/onestep-feishu-bitable/tests
uv build --package onestep-feishu-bitable --out-dir dist/plugin --sdist --wheel --clear
uvx twine check dist/plugin/*
```

push 到 `main` 时，workflow 比较当前提交和 `github.event.before` 中
`plugins/onestep-feishu-bitable/pyproject.toml` 的 `project.version`。只有插件版本
发生变化时才执行 PyPI 发布。

手动 `workflow_dispatch` 可设置 `publish_pypi=true` 发布当前插件版本；如果同时提供
`version`，必须与当前插件版本一致。

默认使用插件专用 secret `FEISHU_BITABLE_PYPI_API_TOKEN`。如果未配置，则回退到已有的
`PYPI_API_TOKEN` 或 `PYPI_TOKEN`。
