...
2026-06-09 后续调整：Feishu Bitable 已按插件方向迁出 core，当前实现位于
`plugins/onestep-feishu-bitable`，详见
`docs/superpowers/specs/2026-06-09-feishu-bitable-plugin-design.md`。
...
每个模块只负责 YAML handler 注册，不改变 connector 类所在位置。Feishu Bitable
例外：它已迁移到 `plugins/onestep-feishu-bitable` 插件包，connector 和 YAML
handler 都由插件提供。
...
- 将 Feishu Bitable 拆到 `onestep-feishu-bitable`
