...
6. **重叠表对并发 `open()` 的建表竞态（F2，Phase 2 验证期观察）。** 触发形态：两组**共享同一 `executions` 表、`attempts` 表不同**的表对**并发** `open()`。实测 10/10 失败（8×1050 表已存在 + 2×1213 死锁）。根因：§6.8 的 DDL `GET_LOCK` 锁名由该表对的两个表名共同派生，重叠表对得到**不同锁名、互不串行**，checkfirst-then-CREATE 随即竞态。对照事实：**同一表对**的并发建表安全（§6.8 的 6-engine 实测 0 错误，即 §14 第 1 条与 §11.2 并发 `auto_create` 判据的场景）；任意表对的**顺序 `open()`** 安全（§11.2 全部场景的实际建表方式）。即 §6.8 的 `GET_LOCK` 方案在其承诺的“同表对串行化”范围内仍然成立，本条是其范围外的组合性观察。该场景**超出 §14 验收判据**（§14 第 1 条只约束同表对并发），不阻塞当前实现。处置三选一，留待全分支评审裁决：(a) 追加二级固定名 `GET_LOCK`，把全部建表 DDL 串行到同一把锁；(b) 文档化「重叠表对必须顺序初始化」的使用约定；(c) 仅记录为已知约束。若评审裁决需要代码修复，将以 repair 轮跟进。
   **裁决：已采纳 (a)，并随 repair 轮 2 实现（本条已解决、关闭）。** 全分支评审 r1 补充说明将本条从「接受 (c)」升级为「建议 (a) 并与 F1/F2 合并进同一 repair 轮」，理由：受影响配置（共享 executions 表 + 多个 attempts 表）是 §6.6/§6.11/§11.2 明确支持并已测试的模式；失败率 10/10 近乎确定（`asyncio.gather` 并发 `open()` 是 Python API 的自然写法）；修复纯增量约十余行。实现：`mysql/execution_schema.py` 新增固定全局锁名 `_MYSQL_EXECUTION_DDL_GLOBAL_LOCK = "lock_onestep_execution_ddl"`（26 字符，遵守 §6.8 的 64 字符上限）；`MySQLExecutionDialect.create_tables` 在同一连接上按**严格固定顺序**先取全局锁、再取表对锁（全局→表对；MySQL ≥5.7.5 允许会话同时持有多把命名锁，本项目下限 8.0.16，且全局锁先行使等待图保持为无环星形），`finally` 中按**相反顺序**释放（表对→全局）；表对锁的派生名与 10s 超时**原样保留**，§6.8 已测行为不变。代价说明：全部 execution 建表 DDL 全局串行——DDL 稀少且毫秒级、非重叠表对的顺序初始化路径不变，可接受。新增 live 回归：两组重叠表对 `asyncio.gather` 并发 `open()` 双双成功，且共享 `executions` 表经 `information_schema` 断言恰建一次。上方 10/10 实测证据保留，作为修复前的复现记录。
