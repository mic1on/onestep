...
校验行按 `batch_size` 分块，每块一条 executemany 写入临时表，因此往返次数与
**块数**成正比而非行数（#228 之前是每行一次执行，2216 行的字符串键批次在 61ms
链路上要等 2216 次往返、耗时 136s，现在约 0.5s）。这条路径用 executemany 而不是
内联多值 `VALUES`：MySQL 包大小守卫对 executemany 逐行度量，对内联多值语句整体
度量，而守卫抛出的是 PERMANENT 错误、不会像真实写入路径那样拆分重试，所以内联
写法会把本可成功的批次判为失败。
...
转义后计算实际查询字节数（含 COM_QUERY 字节）。尚未发送的超限多行语句继续拆分；
单行仍超限则抛出 `PERMANENT` connector 错误，并回滚前面的 chunk。

检查覆盖批内唯一校验的输入语句。使用 asyncmy 的实际 `mogrify` 和连接编码，
包含原生 JSON 序列化及连接的转义模式。断线错误仍按原有规则处理，不能把所有
2013 错误都视为 payload 错误。永久 connector 错误不会触发 sink 内部重试；
外层任务是否重试仍取决于 task retry policy。
