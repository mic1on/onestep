...
本文说明业务系统如何把 MySQL 用作长任务的提交、状态、结果、取消和租约存储。能力与 [PostgreSQL Tracked Execution](/broker/postgres-execution) 逐条同构：同一套 `ExecutionClient` API、同一套状态机、同一组 YAML 字段语义，只替换数据库与驱动层。
...
`onestep-sql[mysql]` 依赖 `onestep>=1.9.0`。反过来，只安装 `onestep==1.9.0` 不会自动启用 tracked execution；没有安装 SQL plugin 的普通 worker 可以照常运行。
...
- [ ] PyPI 中同时存在 `onestep==1.9.0` 和 `onestep-sql==0.4.0`（含 MySQL 后端）。
