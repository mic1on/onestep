...
内置连接器会按稳定类型名上报，例如 `cron`、`interval`、`rabbitmq_queue`、`mysql_table_sink`、`redis_stream` 和 `http_sink`。从 1.2.6 开始，Redis Streams 会上报 stream、group、consumer、batch、block、trim 等拓扑配置；`HttpSink` 会上报 URL、method、timeout 和成功状态码。

敏感信息不会原样进入拓扑：`HttpSink` 上报时会隐藏 URL 账号信息、移除 query/fragment，并把 header 值标记为 `<redacted>`。
