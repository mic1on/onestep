...
- **The rules are wired and validated.** `monitoring/prometheus/prometheus.yml` declares `rule_files` and defines the three scrape jobs the rules select on (`onestep-control-plane`, `-readyz`, `-postgres`); `docker-compose.monitoring.yml` runs Prometheus + Alertmanager + blackbox + postgres_exporter. The `/metrics` endpoint needs a bearer token, so the compose entrypoint writes the first `ONESTEP_CP_INGEST_TOKENS` value to `/etc/prometheus/ingest_token` for `credentials_file` (Prometheus cannot expand env vars in its own config). Run `bash scripts/check-monitoring.sh` after touching any of these; it is also a CI job (`monitoring`).
- Rule job names are load-bearing: a rule selecting on a misspelled job is valid PromQL, loads fine, and never fires. `check-monitoring.sh` and `test_prometheus_exporter.py` both assert every `job="..."` in a rule is defined in the scrape config.
- `monitoring/alertmanager/alertmanager.yml` owns routing (critical pages hourly, warning tickets every 12h) and inhibition. Most alerts are derived from a few root causes, so `ApiDown` inhibits its eleven consequences and `PostgresDown` inhibits only the database-derived ones. Set your webhook in the `default-webhook` receiver; the shipped default is an unresolvable `.invalid` host so a forgotten config fails loudly.
- `critical` means one of two things: the control plane is down, OR it is up but has lost the ability to report problems (`ScanFailing`, `NotificationDeliveryFailures`, `MetricsMissing`, `ScanNeverRan`). The second class is the more dangerous one — a silent alerting system beats a loud outage at hiding incidents. `warning` is degradation that leaves the system observable.
- **`absent()` needs `and on()`.** `absent()` returns an EMPTY label set and a bare `and` matches on all labels, so `up{job="x"} and absent(...)` matches nothing and never fires — a guard that looks like coverage but is not. Verified against Prometheus 2.53; `test_absence_guards_use_and_on` pins it. The four guards in `onestep-control-plane-absence` are gated on `up == 1`, which makes them mutually exclusive with `ApiDown` by construction (so they need no inhibit entry).
- A wrong `/metrics` bearer token makes the target **down** (`up == 0`), so it raises `ApiDown`, not `MetricsMissing` — verified live, and worth remembering before "fixing" the guard's description again.
- `onestep_task_succeeded_total` / `_failed_total` / `_custom_counter_total` keep a `_total` suffix but are exported as **gauges**, because their value is `SUM(TaskMetricWindow.*)` over retained windows and the retention worker deletes windows older than 90 days — the sum DECREASES on every retention pass (proven in `test_task_metric_windows_are_not_monotonic_counters`). Prometheus reads a counter decrease as a reset and folds the whole drop into the next `increase()`, so no rule may use `increase()`/`rate()` on them; `test_no_alert_rule_uses_increase_or_rate_on_task_totals` enforces that. This is why there is no task failure-rate alert: a real one needs a monotonic per-process counter, not a retention-window sum.
- Notification delivery has two severities of failure, kept as separate series on purpose: `failed` counts every failed **attempt** (including retryable ones), `permanently_failed` means the retry budget is spent and that notification is **lost** — the operator was never told. Both count; a permanent failure increments both. `OneStepControlPlaneNotificationLost` inhibits `...DeliveryFailures` so one lost notification does not page twice.
