...
- `monitoring/alertmanager/alertmanager.yml` owns routing (critical pages hourly, warning tickets every 12h) and inhibition. Most alerts are derived from a few root causes, so `ApiDown` inhibits its ten consequences and `PostgresDown` inhibits only the database-derived ones. Set your webhook in the `default-webhook` receiver; the shipped default is an unresolvable `.invalid` host so a forgotten config fails loudly.
