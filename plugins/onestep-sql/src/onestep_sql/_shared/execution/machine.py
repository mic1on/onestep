"""Backend-neutral tracked-execution state machine.

This module holds the single implementation of the tracked-execution state
machine that used to live in ``onestep_sql.postgres.execution_backend`` — it is
Phase 1 of ``docs/superpowers/specs/2026-09-15-mysql-tracked-execution-backend-design.md``
(§7.2–§7.4).

What lives here: submit, idempotent replay/conflict detection, get, keyset
``list`` pagination, cancel, ``FOR UPDATE SKIP LOCKED`` claim, lease heartbeat,
completion with fenced compare-and-set, release, expired-lease reclaim and the
stale-token fencing that guards all of it.

What does *not* live here: the DDL (each backend keeps its own
``execution_schema.py``), the driver mapping, the connector class and the
source/delivery pair. Those are supplied by the concrete backend through
:class:`~onestep_sql._shared.execution.dialect.ExecutionDialect` and two small
subclass hooks (``_make_connector`` / ``_make_source``).

A concrete backend in this repository is a thin subclass: it keeps its public
name, import path and signatures, and provides only its dialect adapter plus
those two hooks. See ``onestep_sql.postgres.execution_backend``.

This package is **not** public API (design §12.3).
"""
...
from .dialect import ExecutionDialect, ExecutionTables, _aware_utc, _optional_aware_utc


class StaleExecutionLease(ExecutionLeaseLost):
    """Raised when a lease token/attempt is no longer authoritative.

    Concrete backends re-export this exact object so that
    ``onestep_sql.postgres.execution_backend.StaleExecutionLease`` keeps its
    identity (design §7.4, §12.2).
    """


class ExecutionStateMachine(LeasedExecutionBackend):
    """Shared implementation of ``LeasedExecutionBackend`` over one SQL pair.

    Subclasses supply the dialect adapter and the two factory hooks. Everything
    else — including the compare-and-set predicates that define the
    at-least-once contract — is shared, because the two backends' SQL is
    isomorphic here (design §7.3).
    """

    #: Human-readable connector class used in the process-boundary error text.
    _connector_hint: ClassVar[str] = "connector"

    #: Subclasses point this at their :class:`ExecutionDialect` implementation.
    #: Keeping it a class attribute (rather than an ``__init__`` parameter)
    #: lets each backend inherit the frozen public constructor signature.
    _dialect_cls: ClassVar[type]
    ...
    def __init__(
        ...
        **engine_options: Any,
        ...
        self._dialect: ExecutionDialect = type(self)._dialect_cls()
        self._dsn = dsn
        self._engine_options = dict(engine_options)
        self._connector = connector
        self._owns_connector = dsn is not None
        self._pid = os.getpid()
        self.tables: ExecutionTables = self._dialect.build_tables(
    ...
    # -- ExecutionBackend -------------------------------------------------
    # Each public method is the frozen, backend-neutral signature from core's
    # ``LeasedExecutionBackend``; the SQL lives in the private implementation.
    ...
    async def _transaction_now(self, conn: Any) -> datetime:
        # Backends whose database clock is stable inside a transaction (for
        # example PostgreSQL's transaction_timestamp()) may derive ``now`` from
        # the connection; every other case uses the injected clock so that all
        # lease checks in one transaction agree on a single instant.
        derived = await self._dialect.transaction_now(conn)
        if derived is None:
            return self._now()
        return _aware_utc(derived)
    ...
    def _ensure_connector(self) -> Any:
        ...
        if self._connector is None:
            ...
            self._connector = self._make_connector(self._dsn, self._engine_options)
        return self._connector
    ...
    def _fork_error(self, current_pid: int) -> RuntimeError:
        return RuntimeError(
            f"{type(self).__name__} cannot reuse an externally supplied connector "
            f"across a process boundary (created in pid {self._pid}, current pid "
            f"{current_pid}); create the {self._connector_hint} in the child process"
        )
    ...
    async def _ensure_ready_locked(self) -> None:
        ...
        if self.auto_create:
            await self._dialect.create_tables(
                engine,
                [self.tables.executions, self.tables.attempts],
            )
        else:
            ...
            async with self.engine.begin() as conn:
                ...
                values = {
                    ...
                    "expires_at": (
                        ...
                        else self._dialect.normalize_datetime(request.expires_at)
                    ),
                }
    ...
    def _decode_cursor(self, cursor: str) -> tuple[datetime, UUID]:
        try:
            ...
            if cursor != type(self)._encode_cursor(created_at, execution_id):
                raise ValueError("invalid execution cursor")
