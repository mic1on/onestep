"""Shared tracked-execution source and delivery.

Phase 1 of ``docs/superpowers/specs/2026-09-15-mysql-tracked-execution-backend-design.md``
(§7.2–§7.4) extracts the source/delivery pair together with the state machine.
The measured duplication justified it (design §15.2 left the decision to Phase 1
measurement): of the original ~470 lines, only two things differ per backend —
the queue name prefix (``postgres.execution:<namespace>``) and the dialect
error-normalization factory. Everything else — option validation, the claim
loop, envelope construction with ``onestep.execution`` correlation metadata,
the heartbeat loop with retry-while-lease-remains, cooperative cancellation,
critical-section shielding around completion, and the ``ack``/``retry``/``fail``
mapping — is identical.

A concrete backend subclasses :class:`ExecutionSourceBase` and
:class:`ExecutionDeliveryBase`, binds those two seams, and keeps its own public
name and import path (design §4.2: backend-named API, never a generic
``SQLExecutionSource``).
...
class ExecutionSourceBase(Source):
    """Shared ``Source`` implementation for a tracked-execution backend.

    Subclasses bind three class attributes:

    * ``_source_kind`` — queue name prefix, e.g. ``"postgres.execution"``;
    * ``_backend_cls`` — that backend's execution backend class;
    * ``_delivery_cls`` — that backend's delivery class;
    * ``_error_factory`` — that backend's ``as_<backend>_connector_operation_error``.
    """

    fetch_is_cancel_safe = False

    _source_kind: ClassVar[str]
    _backend_cls: ClassVar[type]
    _delivery_cls: ClassVar[type]
    _error_factory: ClassVar[Callable[..., ConnectorOperationError | None]]
    ...
    def __init__(
        ...
        **engine_options: Any,
        ...
        super().__init__(f"{self._source_kind}:{namespace.strip()}")
    ...
    def from_connector(
        ...
        *,
        ...
        backend = cls._backend_cls.from_connector(
            connector,
            table=table,
            attempts_table=attempts_table,
            auto_create=auto_create,
    ...
    async def fetch(self, limit: int) -> list[Delivery]:
        try:
            ...
            connector_error = self._error_factory(
...
class ExecutionDeliveryBase(Delivery):
    ...
    cooperative-cancellation semantics are identical on both backends
    (design §7.2).
    """
        ...
        while True:
            try:
                ...
                connector_error = (
                    ...
                    else self.source._error_factory(
