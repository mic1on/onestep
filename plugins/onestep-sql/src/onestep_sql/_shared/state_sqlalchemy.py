"""Shared SQLAlchemy state/cursor stores for the onestep-sql backends.

Phase 2 of the mysql/postgres consolidation (issue #133, design §3.1) keeps
exactly one copy of the store implementation that previously lived in parallel
in both backend ``state_sqlalchemy.py`` modules (see the retired
``scripts/check_plugin_drift.py``). Dual-backend contract tests in
``tests/contract/test_onestep_sql_shared.py`` pin the behaviour for both
backends.

Only two things are genuinely per-database and stay in the backend adapters:

* the asyncio driver mapping (``mysql+asyncmy`` vs ``postgresql+psycopg``) —
  backends override :meth:`SQLAlchemyStateStore._resolve_async_driver`;
* the installation hint in the missing-SQLAlchemy error message — backends
  override ``_install_hint``.

Backends subclass :class:`SQLAlchemyStateStore` /
:class:`SQLAlchemyCursorStore` so their public class identities stay inside
``onestep_sql.mysql`` / ``onestep_sql.postgres`` while the behaviour is
...
def sqlite_async_driver(drivername: str) -> str | None:
    """Return the asyncio sqlite driver for sqlite URLs, else ``None``."""
    if drivername in ("sqlite", "sqlite+pysqlite"):
        return "sqlite+aiosqlite"
    return None


def async_dsn(dsn: str, *, resolve_driver: Any) -> str:
    """Return a SQLAlchemy URL backed by an asyncio-compatible dialect.

    ``resolve_driver`` maps the URL's current drivername to the
    asyncio-compatible drivername to use, or ``None`` to keep the DSN
    unchanged. The per-database mappings live in the backend adapters.
    """
    if sa is None:
        return dsn
    url = sa.engine.make_url(dsn)
    async_drivername = resolve_driver(url.drivername)
    if async_drivername is None:
        return dsn
    return url.set(drivername=async_drivername).render_as_string(hide_password=False)


class SQLAlchemyStateStore:
    _install_hint = "Install onestep-sql."
    ...
    def __init__(
        ...
        **engine_options: Any,
        ...
        if create_async_engine is None or sa is None:
            raise RuntimeError(f"SQLAlchemyStateStore requires SQLAlchemy. {self._install_hint}")
        ...
        self.engine = engine or create_async_engine(
            async_dsn(dsn, resolve_driver=self._resolve_async_driver), **engine_options
        )
