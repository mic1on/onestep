...
def test_postgres_incremental_cursor_advances_in_order(tmp_path: Path) -> None:
    ...
    async def scenario() -> None:
        ...
        async with restarted_db.engine.begin() as conn:
...
def test_postgres_incremental_uses_key_as_tie_breaker_when_cursor_is_not_unique(tmp_path: Path) -> None:
    ...
    async def scenario() -> None:
        ...
        async with db.engine.begin() as conn:
            await conn.execute(sa.insert(users), [{"id": 3, "name": "C", "updated_at": 10, "deleted": 0}])
...
def test_postgres_incremental_retry_redelivers_same_row_with_incremented_attempts(
    tmp_path: Path,
) -> None:
...
def test_postgres_incremental_retry_delay_and_inflight_gap_pause_sql_reads(
    tmp_path: Path,
) -> None:
...
def test_postgres_incremental_terminal_failure_blocks_before_failed_cursor(
    tmp_path: Path,
) -> None:
...
def test_postgres_incremental_coalesces_concurrent_contiguous_acks(tmp_path: Path) -> None:
...
def test_postgres_incremental_cursor_save_failure_preserves_retryable_prefix(
    tmp_path: Path,
) -> None:
...
def test_postgres_incremental_logs_fetch_retry_commit_without_sensitive_values(
    tmp_path: Path, caplog: pytest.LogCaptureFixture
) -> None:
    db_url = f"sqlite:///{tmp_path / 'inc-log.db'}"
    engine = sa.create_engine(db_url, future=True)
    metadata = sa.MetaData()
    rows = sa.Table(
        "rows",
    ...
    } <= records["postgres_incremental_fetch"].__dict__.keys()
    assert {
        "retry_count",
        "attempt",
        "delay_s",
        "pending_cursor_rows",
    } <= records["postgres_incremental_retry"].__dict__.keys()
    assert {
        "cursor_save_count",
        "coalesced_ack_count",
        "duration_s",
        ...
        "pending_cursor_rows",
        "fetched_cursor_lag_rows",
    } <= records["postgres_incremental_cursor_commit"].__dict__.keys()
...
def test_postgres_incremental_restarts_from_datetime_cursor(tmp_path: Path) -> None:
