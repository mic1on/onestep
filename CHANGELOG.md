...
批内唯一校验的往返次数从「每行一次」降为「每块一次」（#228）。

- **临时校验表改为每块一条 executemany 灌入。** 0.7.0 逐行执行 `INSERT`，于是预检成本随行数线性增长：字符串键的 2216 行批次要付 2216 次往返——在 61ms 链路上耗时 136s，而它所守护的多值 upsert 本身只要 1.9s。临时表不复制默认值、序列、触发器或目标数据，两种写法插入的值完全相同、冲突判定同样交给数据库，差别只在往返次数。同一批次现在只需 3 次往返、约 0.5s，结果逐字节一致。

- **合并写入用 executemany，而不是内联多值 `VALUES`。** MySQL 包大小守卫对 executemany 逐行度量、对内联多值语句整体度量，且抛出的是 `PERMANENT` 错误、不做拆分重试；内联写法因此会把真实写入路径本可拆分处理的批次判为失败。传参数映射列表可以保持守卫的逐行视角与原有行为。

- 整数快速通道不变：它只覆盖索引列全为整数类型、且取值全为 Python `int` 的情形，字符串业务键仍走数据库校验。#222 的全部保证保持不变——覆盖排序规则冲突、普通 NULL 重复、`NULLS NOT DISTINCT`、部分索引谓词与跨块冲突的回归集原样通过。
