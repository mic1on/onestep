...
def test_build_worker_package_collects_code_dependencies_and_manifest_includes(tmp_path):
    ...
    (tmp_path / "README.md").write_text("# build demo\n", encoding="utf-8")
...
def test_build_worker_package_collects_conditional_emit_string_predicates(tmp_path):
    (tmp_path / "worker.yaml").write_text(
        "\n".join(
            [
                "apiVersion: onestep/v1alpha1",
                ...
                "    source: tick",
                "    emit:",
                "      - when: route_predicates:is_active",
                "        then: out",
                "",
            ]
        ),
    ...
    (tmp_path / "route_predicates.py").write_text(
    ...
    output = tmp_path / "dist" / "worker.zip"
    result = build_worker_package(
        BuildOptions(target=str(tmp_path / "worker.yaml"), output=str(output), strict=True)
    )

    assert "route_predicates.py" in result.files
    with zipfile.ZipFile(output, "r") as archive:
        assert "route_predicates.py" in archive.namelist()


def test_build_worker_package_includes_pyproject_referenced_metadata_files(tmp_path):
    (tmp_path / "pyproject.toml").write_text(
        "\n".join(
            [
                "[project]",
                ...
                'license-files = ["NOTICE", "licenses/THIRD_PARTY*"]',
    ...
    (tmp_path / "docs").mkdir()
    (tmp_path / "docs" / "README.md").write_text("# metadata demo\n", encoding="utf-8")
    (tmp_path / "NOTICE").write_text("notice\n", encoding="utf-8")
    (tmp_path / "licenses").mkdir()
    (tmp_path / "licenses" / "LICENSE.txt").write_text("MIT\n", encoding="utf-8")
    (tmp_path / "licenses" / "THIRD_PARTY.txt").write_text("third-party\n", encoding="utf-8")
    src_dir = tmp_path / "src" / "metadata_worker"
    src_dir.mkdir(parents=True)
    (src_dir / "__init__.py").write_text("", encoding="utf-8")
    (src_dir / "tasks.py").write_text(
        "async def handle(ctx, item):\n    return None\n",
    ...
    (tmp_path / "worker.yaml").write_text(
        "\n".join(
            [
                ...
                "    source: tick",
                "    handler:",
                "      ref: metadata_worker.tasks:handle",
                "",
            ]
        ),
        encoding="utf-8",
    ...
    assert "docs/README.md" in result.files
    assert "licenses/LICENSE.txt" in result.files
    assert "NOTICE" in result.files
    assert "licenses/THIRD_PARTY.txt" in result.files
    assert result.warnings == []
